...
当前状态：进行中
当前进展：共享工具层已经开始从 `Claude SDK` 链路中抽离。当前新增了 `src/gateway/tools/*` 初版 registry，已将 `local-tools / web-tools / ai-browser / skill / extension MCP` 的组装逻辑从 `src/main/services/agent/sdk-options.ts` 中迁出，变成可复用的 `buildToolRegistry()` 边界；同时，`local MCP` 工具的共享 catalog type 和第一批 permission policy 已经从 `tool-catalog / permission-handler` 中抽出。现在完整的 `tool catalog` 已经迁入 `src/gateway/tools/catalog.ts`，`tool registry` 也开始返回共享 `provider definitions`、`catalog` 和 `directory`，`sdk-mcp-server` 已直接依赖共享 catalog，native lane 导出的 function tools 也开始优先复用同一份共享工具说明，不再自己拼另一套文案。并且第一版任务级 runtime 路由 pure helper 已经落地：`hybrid` 模式下会把轻任务倾向到 `native`、复杂编排任务倾向到 `claude-sdk`，`Ralph` 和 hosted subagent auto-announce 也已经开始透出显式 `runtimeTaskHint`。最新一轮又把一组最容易重复维护、最容易漏改的清单继续收进了 `gateway/tools`：Claude Code SDK 默认允许的 built-in tools、平台统一禁用的 server-side tools、以及 hosted subagent 不允许使用的 built-in tools，现在都由同一份 `src/gateway/tools/built-ins.ts` 提供；而新补上的 `src/gateway/tools/directory.ts` 又把“工具属于谁、哪条 runtime 能用、需不需要确认”这些运行时元信息整合进一份共享工具目录。`sdk-options`、`permission-handler`、`subagent/runtime` 与 native function tool exporter 已不再各自维护一份副本。`Claude SDK Runtime` 现阶段仍然维持原行为，但工具描述、provider 元数据、审批策略、built-in capability 清单、工具目录和 runtime 路由入口已经开始沿共享工具层收口。这意味着后续做 `NativeRuntime` 时，不需要再从 `sdk-options`、`tool-catalog`、`permission-handler`、`subagent/runtime` 或 native exporter 里复制一套工具装配和边界控制逻辑，而是直接复用 `gateway/tools` 和 `gateway/runtime/routing.ts`。
