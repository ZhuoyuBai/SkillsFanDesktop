...
> **SkillsFan** は Claude Code CLI をビジュアル AI コンパニオンに変えます。
...
### 本物の Agent ループ
チャットだけではありません。SkillsFan は **実際に行動できます** — コードを書き、ファイルを作成し、コマンドを実行し、タスクが完了するまで反復します。
...
### リモートアクセス
スマートフォンや任意のブラウザからデスクトップの SkillsFan を制御。どこからでも作業 — 病院のベッドからでも（実話）。
...
1. **SkillsFan を起動** して API キーを入力（Anthropic 推奨）
...
SkillsFan は開発者だけのものではありません。私たちが見てきたもの：
...
障壁はもはや AI の能力ではありません。**アクセシビリティです。** SkillsFan はその障壁を取り除きます。
...
SkillsFan が素晴らしいものを作る手助けになったら、ぜひ聞かせてください。

**このリポジトリに Star を** して、他の人が SkillsFan を発見する手助けをしてください。
