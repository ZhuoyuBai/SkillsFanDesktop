...
function getActivityText(thought: Thought, t: (key: string, params?: Record<string, unknown>) => string): string {
  ...
  switch (thought.toolName) {
    ...
    case 'mcp__local-tools__code_execution':
      return t('Running {{command}}...', { command: truncate(String(input?.language || 'code'), 20) })
    case 'WebFetch':
    ...
    case 'mcp__local-tools__memory':
      return t('Searching {{query}}...', { query: truncate(String(input?.query || input?.path || 'memory'), 20) })
    case 'mcp__local-tools__text_editor_code_execution':
      return t('Editing {{file}}...', { file: extractFileName(input?.path) })
    case 'mcp__local-tools__tool_search_tool_regex':
    case 'mcp__local-tools__tool_search_tool_bm25':
      return t('Searching {{pattern}}...', { pattern: truncate(String(input?.pattern || input?.query || ''), 20) })
    case 'TodoWrite':
...
function getCompletedText(thought: Thought, t: (key: string, params?: Record<string, unknown>) => string): string {
  ...
  switch (thought.toolName) {
    ...
    case 'mcp__local-tools__code_execution':
      return t('Ran {{command}}', { command: truncate(String(input?.language || 'code'), 20) })
    case 'WebFetch':
    case 'mcp__web-tools__WebFetch':
    ...
    case 'mcp__local-tools__memory':
      return t('Searched {{query}}', { query: truncate(String(input?.query || input?.path || 'memory'), 20) })
    case 'mcp__local-tools__text_editor_code_execution':
      return t('Edited {{file}}', { file: extractFileName(input?.path) })
    case 'mcp__local-tools__tool_search_tool_regex':
    case 'mcp__local-tools__tool_search_tool_bm25':
      return t('Searched {{pattern}}', { pattern: truncate(String(input?.pattern || input?.query || ''), 20) })
    case 'TodoWrite':
