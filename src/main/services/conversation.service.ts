...
function getThoughtPreview(thoughts?: Thought[]): string | undefined {
  if (!thoughts || thoughts.length === 0) return undefined

  for (let i = thoughts.length - 1; i >= 0; i -= 1) {
    const thought = thoughts[i]
    if (thought.type === 'error' && thought.content) {
    ...
    if (thought.type === 'tool_use' && thought.toolName) {
      return `Tool: ${thought.toolName}`
    }
    if (thought.type === 'tool_result' && thought.toolName) {
      return `Result: ${thought.toolName}`
    }
  }
...
function toMeta(conversation: Conversation): ConversationMeta {
  ...
  if (lastMessage) {
    const content = lastMessage.role === 'assistant'
      ? stripLeadingSetModelStatus(lastMessage.content || '')
      : lastMessage.content
    const rawPreview = content || getThoughtPreview(lastMessage.thoughts) || ''
    preview = rawPreview.slice(0, PREVIEW_LENGTH)
    if (rawPreview.length > PREVIEW_LENGTH) {
      preview += '...'
    }
  }
