...
SkillsFan is a cross-platform desktop application that wraps Claude Code in a visual interface. Built with Electron + React + TypeScript using `electron-vite`, it provides a GUI alternative to the CLI while maintaining full agent capabilities.
...
- `patches/` - patch-package patches (notably for `@anthropic-ai/claude-agent-sdk`)
...
The `@anthropic-ai/claude-agent-sdk` is patched via `patches/` — check the patch file when upgrading the SDK.
...
Run `npm run i18n` after adding new UI text. This extracts keys and auto-translates via Claude API.
